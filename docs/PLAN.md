...
- **Fallback**: cualquier error de red, timeout o respuesta no-200/JSON inválido dispara el fallback a la última entrada de `data/history.json`. Si no hay historial previo y la API falla, se informa el error explícitamente (no hay con qué generar el reporte).
